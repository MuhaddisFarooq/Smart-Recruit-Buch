...
export const getActiveNav = () => {
  ...
  const processNavItems: any = (items: any[]) => {
    return items.map((item) => {
      let subItems = item.items ? processNavItems(item.items) : [];

      const isActive = role === superAdmin
        ? true
        : isPageAllowed(item.url) || subItems.some((sub: any) => sub.isActive);
