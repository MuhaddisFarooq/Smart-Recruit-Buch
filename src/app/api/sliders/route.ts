...
export async function GET(req: NextRequest) {
  try {
    ...
    const where: string[] = [];
    const args: any[] = [];
    if (search) { where.push("(status LIKE ?)"); args.push(`%${search}%`); }
    const whereSql = where.length ? `WHERE ${where.join(" AND ")}` : "";

    const totalRows = await query<{ cnt: number }>(
      `SELECT COUNT(*) as cnt FROM sliders ${whereSql}`, args
    );
