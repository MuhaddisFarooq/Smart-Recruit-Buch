// src/app/api/insurance/company/[id]/route.ts
...
export async function GET(
  ...
  try {
    ...
    const rows = await query<any>(
      "SELECT * FROM insurance_company WHERE id=? LIMIT 1",
      [num]
    );
    if (!rows?.length) {
    ...
    console.error("[insurance_company:GET one] DB error:", err);
...
// PATCH /api/insurance/company/[id]
  ...
  try {
    ...
    if (ct.includes("multipart/form-data")) {
      ...
      if (logoFile && logoFile.size > 0) {
        ...
        const saved = await saveCompressedJpeg(logoFile, "company");
        logoRel = saved ?? undefined;

        // delete old file, if any
        const old = await query<{ logo: string | null }>(
          "SELECT logo FROM insurance_company WHERE id=? LIMIT 1",
          [num]
        );
    ...
    // Always track updater & time if something else is being updated
    if (sets.length === 0) {
    ...
    await query(
      `UPDATE insurance_company SET ${sets.join(", ")} WHERE id=?`,
      [...args, num]
    );

    return NextResponse.json({ ok: true });
  } catch (err: any) {
    console.error("[insurance_company:PATCH] DB error:", err);
    return NextResponse.json(
      { error: err?.message || "DB error" },
      { status: 500 }
    );
...
// DELETE /api/insurance/company/[id]
export async function DELETE(
  ...
  try {
    ...
    await query("DELETE FROM insurance_company WHERE id=?", [num]);
    return NextResponse.json({ ok: true });
  } catch (err: any) {
    console.error("[insurance_company:DELETE] DB error:", err);
    return NextResponse.json(
      { error: err?.message || "DB error" },
      { status: 500 }
    );
