// src/app/(dashboard)/insurance/company/[id]/edit/page.tsx
"use client";
...
export default function EditInsuranceCompanyPage() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/insurance/company/${id}`, { cache: "no-store" });
        ...
        notify.error(e?.message || "Failed to load company.");
      } finally {
        setLoading(false);
      }
    })();
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const task = (async () => {
        const r = await fetch(`/api/insurance/company/${id}`, {
          method: "PATCH",
          body: fd,
        });
      ...
      router.push("/insurance/company/view");
    } catch {
      // toast already shown
    } finally {
      setSaving(false);
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-semibold">Edit Insurance Company</h1>
