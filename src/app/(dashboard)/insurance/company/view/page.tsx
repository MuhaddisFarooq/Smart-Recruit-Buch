...
export default function ViewInsuranceCompanyPage() {
  ...
  async function loadAt(p = page, ps = pageSize, q = search) {
    ...
    try {
      ...
      const res = await fetch(`/api/insurance/company?${params.toString()}`, { cache: "no-store" });
  ...
  useEffect(() => {
    const t = setTimeout(() => { setPage(1); loadAt(1, pageSize, search); }, 350);
  ...
  async function onDelete(id: number) {
    ...
    try {
      const task = (async () => {
        const res = await fetch(`/api/insurance/company/${id}`, { method: "DELETE" });
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-4 py-6 text-center text-gray-500">No companies found.</td></tr>
            ) : rows.map((r, i) => (
