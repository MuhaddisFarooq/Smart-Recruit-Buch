// src/app/(dashboard)/insurance/corporate/add/page.tsx
"use client";

import { useState } from "react";
import { notify } from "@/components/ui/notify";

export default function AddInsuranceCorporatePage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      setSaving(true);
      const task = (async () => {
        const res = await fetch("/api/insurance/corporate", { method: "POST", body: fd });
      ...
      await notify.promise(task, {
        loading: "Saving corporate…",
        success: "Corporate saved.",
  ...
  return (
    <div className="p-6">
      <h1 className="mb-2 text-2xl font-semibold">Add Insurance Corporate</h1>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2">
            ...
            <textarea className="w-full rounded-md border px-3 py-2" rows={4}
              value={form.profile} onChange={(e) => setForm({ ...form, profile: e.target.value })}
