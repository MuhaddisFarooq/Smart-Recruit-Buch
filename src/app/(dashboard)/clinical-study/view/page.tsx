...
export default function ClinicalStudyViewPage() {
  ...
  async function loadAt(p = page, ps = pageSize, term = search) {
    ...
    try {
      ...
      const res = await fetch(`/api/clinical-study?${params.toString()}`, { cache: "no-store" });
      ...
      console.error(e);
      notify.error("Failed to load clinical studies.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function onDelete(id: number) {
    const ok = await confirm({
      title: "Delete this clinical study?",
    ...
    try {
      const task = (async () => {
        const res = await fetch(`/api/clinical-study/${id}`, { method: "DELETE" });
        const j = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(j?.error || `HTTP ${res.status}`);
      })();
      await notify.promise(task, {
  ...
  return (
    <div className="p-6">
      <h1 className="mb-4 text-2xl font-semibold">View Clinical Studies</h1>
      ...
      <div className="mb-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            placeholder="heading or description…"
          />
        </div>
      </div>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-4 py-6 text-center text-gray-500">No clinical studies found.</td></tr>
            ) : rows.map((r, i) => (
              <tr key={r.id} className={i % 2 ? "bg-white" : "bg-gray-50/50"}>
                ...
                <td className="px-4 py-3">
                  {r.picture ? (
                    ...
                    <img src={toUrl(r.picture)} alt={`c-${r.id}`} className="h-12 w-20 rounded border object-cover" />
                  ) : "—"}
                </td>
                <td className="px-4 py-3">{r.heading_name || "—"}</td>
                <td className="px-4 py-3">
                  {r.link ? (
