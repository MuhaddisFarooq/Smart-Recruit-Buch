// src/app/(dashboard)/popup/view/page.tsx
"use client";
...
export default function PopupViewPage() {
  ...
  const canView = hasPerm(perms, "popup", "view");
  const canEdit = hasPerm(perms, "popup", "edit");
  const canDelete = hasPerm(perms, "popup", "delete");
  ...
  async function loadAt(p = page, ps = pageSize, term = search) {
    ...
    try {
      ...
      const res = await fetch(`/api/popups?${params.toString()}`, { cache: "no-store" });
      ...
      console.error(e);
      notify.error("Failed to load popups.");
    } finally {
      setLoading(false);
    }
  }
  ...
  function url(s: string) { return s.startsWith("/") ? s : `/uploads/${s}`; }
  ...
  return (
    <div className="p-6">
      <h1 className="mb-4 text-2xl font-semibold">View PopUps</h1>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-4 py-6 text-center text-gray-500">No popups found.</td></tr>
            ) : rows.map((r, i) => (
              <tr key={r.id} className={i % 2 ? "bg-white" : "bg-gray-50/50"}>
                <td className="px-4 py-3">{r.id}</td>
                <td className="px-4 py-3">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={url(r.image)} alt={`popup-${r.id}`} className="h-12 w-20 rounded border object-cover" />
                </td>
