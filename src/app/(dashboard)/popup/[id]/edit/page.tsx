// src/app/(dashboard)/popup/[id]/edit/page.tsx
"use client";
...
export default function EditPopupPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch(`/api/popups/${id}`, { cache: "no-store" });
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (image) {
        ...
        res = await fetch(`/api/popups/${id}`, { method: "PATCH", body: fd });
      } else {
        res = await fetch(`/api/popups/${id}`, {
          method: "PATCH",
  ...
  return (
    <div className="p-6">
      <h1 className="mb-4 text-2xl font-semibold">Edit PopUp</h1>
