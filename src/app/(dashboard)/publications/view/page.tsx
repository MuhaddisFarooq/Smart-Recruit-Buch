...
export default function PublicationsViewPage() {
  ...
  async function loadAt(p = page, ps = pageSize, term = search) {
    ...
    try {
      ...
      const res = await fetch(`/api/publications?${params.toString()}`, { cache: "no-store" });
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-4 py-6 text-center text-gray-500">No publications found.</td></tr>
            ) : rows.map((r, i) => (
              <tr key={r.id} className={i % 2 ? "bg-white" : "bg-gray-50/50"}>
                ...
                <td className="px-4 py-3">
                  {r.picture ? (
                    ...
                    <img src={toUrl(r.picture)} alt={`p-${r.id}`} className="h-12 w-20 rounded border object-cover" />
                  ) : "—"}
                </td>
