// src/app/(dashboard)/fertility-treatments/view/page.tsx
"use client";
...
export default function FertilityViewPage() {
  ...
  async function loadAt(p = page, ps = pageSize, term = search) {
    ...
    try {
      ...
      const res = await fetch(`/api/fertility-treatments?${params.toString()}`, { cache: "no-store" });
  ...
  async function onDelete(id: number) {
    ...
    try {
      const task = (async () => {
        const res = await fetch(`/api/fertility-treatments/${id}`, { method: "DELETE" });
        const j = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(j?.error || `HTTP ${res.status}`);
      })();
      await notify.promise(task, { loading: "Deleting…", success: "Deleted.", error: (e) => (e as Error)?.message || "Could not delete." });
              ...
              <tr key={r.id} className={i % 2 ? "bg-white" : "bg-gray-50/50"}>
                ...
                <td className="px-4 py-3">
                  {url(r.image) ? (
                    ...
                    <img src={url(r.image)!} alt={r.title} className="h-12 w-16 rounded border object-cover" />
